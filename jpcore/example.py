...
class ExampleSource:
    ...
    @property
    def img_link(self):
        return f"""<a href="{self.url}" target="_blank"><img src="{self.logo_url}" alt="{self.description}" title="{self.description}" style="width:{self.icon_size}px;height:{self.icon_size}px;"></a>"""
    ...
    def of_path(cls,path:str):
        ...
        """
        if issue_match := re.search("issue_([0-9]+)", path):
        ...
        elif stackoverflow_match := re.search("stackoverflow[/]q([0-9]+)", path):
            stackoverflow_number = stackoverflow_match[1]
    ...
    def __str__(self):
        ...
        """
        return f"{self.source_type}:{self.description}({len(self.lines)} lines)"
