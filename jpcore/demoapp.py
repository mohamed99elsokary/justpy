...
class JustpyDemoApp:
    ...
    def __init__(self,examples_dir:str,pymodule_file:str, wpfunc: typing.Callable = None, debug:bool=False,**kwargs):
        ...
        self.source_file = source_file.replace(f"{self.examples_dir}/", "")
    ...
    def check_demo(self):
        ...
        if func_match := re.search(
            ...
            self.description = func_match[1]
            self.wpfunc_name = func_match[2]
            self.wpfunc_params = func_match[3]
            if module_match := re.search("(examples/.*)[.]py", self.pymodule_file):
                self.pymodule = module_match[1]
                self.pymodule = self.pymodule.replace("/", ".")
                self.is_demo = "lambda" not in self.wpfunc_name
    ...
    def video_link(self,target_url:str=None,alt:str=None,title:str=None,video_size=512):
        ...
        return f"""<a href="{target_url}" target="_blank"><img src="{video_url}" alt="{alt}" style="width:{video_size}px;height:{video_size}px;"></a>"""
...
Demo("{self.name}", {args}"""
    ...
    def __str__(self):
        ...
        """
        return f"{self.pymodule}({self.example_source}):{self.wpfunc_name}:{self.description}"
